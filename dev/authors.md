# Authors and Citation

## Authors

- **Sonabend Raphael**. Author.
  [](https://orcid.org/0000-0001-9225-4654)

- **Florian Pfisterer**. Author.
  [](https://orcid.org/0000-0001-8867-762X)

- **Michel Lang**. Contributor.
  [](https://orcid.org/0000-0001-9754-0393)

- **Bernd Bischl**. Contributor.
  [](https://orcid.org/0000-0001-6002-6980)

- **Sebastian Fischer**. Maintainer, contributor.
  [](https://orcid.org/0000-0002-9609-3197)

## Citation

Source:
[`DESCRIPTION`](https://github.com/mlr-org/mlr3benchmark/blob/main/DESCRIPTION)

Raphael S, Pfisterer F (2026). *mlr3benchmark: Analysis and
Visualisation of Benchmark Experiments*. R package version 0.1.7-9000,
<https://mlr3benchmark.mlr-org.com>.

    @Manual{,
      title = {mlr3benchmark: Analysis and Visualisation of Benchmark Experiments},
      author = {Sonabend Raphael and Florian Pfisterer},
      year = {2026},
      note = {R package version 0.1.7-9000},
      url = {https://mlr3benchmark.mlr-org.com},
    }
