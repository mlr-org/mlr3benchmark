...
BenchmarkAggr = R6Class("BenchmarkAggr",
  public = list(
    ...
    #' @description Ranks the aggregated data
    rank_data = function(meas = NULL, task = NULL) {
      ...
      if (is.null(meas)) {
        ranks = lapply(self$measures, .rank)
        names(ranks) = self$measures
        return(ranks)
      } else {
        return(.rank(meas))
      }
    ...
    friedman_posthoc = function(meas, p.value = 0.05) {
      ...
      q.nemenyi = qtukey(1 - p.value, self$nlrns, 1e+06) / sqrt(2L)
      cd.nemenyi = q.nemenyi * sqrt(self$nlrns * (self$nlrns + 1L) / (6L * self$ntasks))
      q.bd = qtukey(1L - (p.value / (self$nlrns - 1L)), 2L, 1e+06) / sqrt(2L)
      cd.bd = q.bd * sqrt(self$nlrns * (self$nlrns + 1L) / (6L * self$ntasks))
