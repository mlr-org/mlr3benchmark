...
**mlr3benchmark** enables fast and efficient analysis of benchmark experiments in just a few lines of code and you are not limited to analysing experiments from [mlr3](https://github.com/mlr-org/mlr3). As long as you can coerce your results into a format fitting our classes (which have very few requirements), then you can perform your benchmark analysis with **mlr3benchmark**. 

## Feature Overview

Currently **mlr3benchmark** only supports analysis of multiple learners over multiple tasks. Below is an example of a complete analysis using a benchmark experiment from [mlr3](https://github.com/mlr-org/mlr3).
...
i) xgboost is significantly better than featureless
ii) xgboost is not significantly better than rpart
iii) rpart is not significantly better than featureless

Now we visualise two much simpler plots which display similar information, the first is the mean and standard error of the results across all tasks, the second is a boxplot results across all tasks:
